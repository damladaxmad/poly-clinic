...
const StoolPrint = (props) => {
    ...
    props.data?.tests?.map(t => {
        if (t.stoolResult?.response.length > 0) data = t?.stoolResult?.response
    })
...
const KeyValue = (props) => {
    ...
    display: "flex",
        padding: "0px"}}>
            <Typography style = {{width: "55%", borderRight: "1px solid black",
        padding: "4px 10px"}}>  {props.title} </Typography>
            <Typography style = {{width: "45%", fontWeight: "bold",
